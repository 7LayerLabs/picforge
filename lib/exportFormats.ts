...
export async function downloadTransparentPNG(
  ...
  try {
    ...
    logger.error('Error creating transparent PNG:', error)
    throw error
  }
}
...
export async function downloadSVG(
  ...
  try {
    ...
    if (!options.vectorize) {
      logger.info('Basic SVG exported with embedded image. Enable vectorization for true vector paths.')
    }
  } catch (error) {
    logger.error('Error creating SVG:', error)
    throw error
  }
}
...
export async function downloadPDF(
  ...
  try {
    ...
    logger.info(`PDF created: ${pageWidth}x${pageHeight}mm, ${pdfOptions.orientation}`)
  } catch (error) {
...
export async function downloadWebP(
  ...
  try {
    ...
    logger.error('Error creating WebP:', error)
    throw error
  }
}
...
export async function downloadICO(
  ...
  try {
    ...
    logger.info(`Icon pack created with sizes: ${sizes.join(', ')}px`)
  } catch (error) {
    logger.error('Error creating ICO:', error)
    throw error
  }
}
...
`
    ...
    logger.error('Error creating export pack:', error)
    throw error
  }
}
