...
export default function BatchPage() {
  ...
  // Get user tier for watermark logic and batch limits
  const { usage, tier, getUserBatchLimit, userHasPriorityQueue } = useImageTracking()
  ...
  // Get the batch limit for the current tier
  const batchLimit = getUserBatchLimit()

  // Helper function to add images with batch limit enforcement
  const addImagesWithLimitCheck = (newFiles: File[]) => {
    setBatchLimitError(null)
    const currentCount = images.length
    const availableSlots = batchLimit - currentCount

    if (availableSlots <= 0) {
      setBatchLimitError(`Batch limit reached! Your ${tier} tier allows ${batchLimit} images per batch. Upgrade for larger batches.`)
      return
    }

    const filesToAdd = newFiles.slice(0, availableSlots)
    const rejected = newFiles.length - filesToAdd.length

    if (rejected > 0) {
      setBatchLimitError(`Added ${filesToAdd.length} images. ${rejected} rejected - ${tier} tier limit is ${batchLimit} images per batch.`)
    }
  ...
  useEffect(() => {
    ...
    const handleDrop = (e: DragEvent) => {
      ...
      if (imageFiles.length > 0) {
        addImagesWithLimitCheck(imageFiles)
      }
    }
  ...
  useEffect(() => {
    const handlePaste = async (e: ClipboardEvent) => {
      ...
      if (files.length > 0) {
        addImagesWithLimitCheck(files)
      }
    }

    document.addEventListener('paste', handlePaste)
    return () => document.removeEventListener('paste', handlePaste)
  }, [images.length, batchLimit])
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    addImagesWithLimitCheck(acceptedFiles)
  }, [images.length, batchLimit])
  ...
  const togglePriority = (id: string) => {
    // Only allow priority toggle for users with priority queue access
    if (!userHasPriorityQueue()) {
      setBatchLimitError('Priority queue is available for Creator tier and above. Upgrade to prioritize images!')
      return
    }
    ...
    for (const image of sortedImages) {
      ...
      try {
        ...
        try {
          ...
          logger.error('Effect application failed:', effectError)
      ...
      {isDraggingPage && (
        <div className="fixed inset-0 z-50 bg-purple-500/20 backdrop-blur-sm flex items-center justify-center pointer-events-none">
          <div className="bg-white rounded-xl p-8 shadow-2xl">
            <Upload className="w-16 h-16 text-purple-500 mx-auto mb-3 animate-bounce" />
            <p className="text-xl font-black text-gray-900 tracking-tight">Drop anywhere!</p>
          </div>
        </div>
      )}
      {/* Header */}
      <div className="border-b bg-white/80 backdrop-blur-sm sticky top-0 z-10 shadow-sm">
        <div className="container mx-auto px-4 py-2.5">
          <div className="flex items-center justify-between">
            <Link
              href="/"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-gray-600 hover:text-purple-600 font-semibold hover:bg-purple-50 rounded-lg transition-all text-sm"
            >
            ...
            <div className="flex items-center gap-4">
              <div className="text-xs text-gray-600">
                {stats.total === 0 ? (
                  <span className="font-bold">Drop files to start</span>
                ) : (
                  <span className="font-bold">{stats.total} loaded • {stats.completed} done</span>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="container mx-auto px-4 py-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-4">
            <h1 className="font-heading text-5xl md:text-6xl font-black text-black mb-3 leading-tight tracking-tight uppercase border-b-4 border-brutal-pink inline-block pb-2">
              The Foundry
            </h1>
            <p className="text-lg text-black max-w-2xl mx-auto font-bold leading-snug">
              <span className="text-brutal-cyan">Up to {batchLimit} images.</span> <span className="text-brutal-pink">One click.</span> <span className="bg-brutal-yellow px-2">Zero patience.</span>
            </p>
            <p className="text-xs text-gray-500 mt-1 font-semibold">
              {tier.charAt(0).toUpperCase() + tier.slice(1)} tier: {batchLimit} images per batch
              {!userHasPriorityQueue() && ' • Priority queue available on Creator+'}
            </p>
          </div>

          {/* Batch Limit Error/Warning */}
          {batchLimitError && (
            <div className="bg-amber-50 border-2 border-amber-300 rounded-lg p-3 mb-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <AlertCircle className="w-5 h-5 text-amber-600" />
                <span className="text-sm font-semibold text-amber-800">{batchLimitError}</span>
              </div>
              <div className="flex items-center gap-2">
                <Link
                  href="/pricing"
                ...
                <button
                  onClick={() => setBatchLimitError(null)}
                  className="p-1 hover:bg-amber-200 rounded transition-all"
                >
          ...
          {images.length === 0 ? (
            <>
              <div
                {...getRootProps()}
                className={`border-3 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all mb-4 shadow-sm ${
                  isDragActive
              ...
              >
                ...
                <div className="max-w-xl mx-auto">
                  <Upload className="w-14 h-14 mx-auto text-purple-500 mb-3" />
                  <p className="text-2xl font-black text-gray-900 mb-1.5 leading-tight tracking-tight">
                    Drop Your Images. Go Wild.
                  </p>
                  <p className="text-sm text-gray-600 mb-4 font-semibold">
                    Drop images or click to browse
                  </p>
                  <div className="inline-flex items-center gap-4 text-xs text-gray-500 font-medium">
              ...
              {/* Example Effects - Preview */}
              <div className="mt-8 max-w-4xl mx-auto">
                <p className="text-sm font-bold text-black mb-3 uppercase tracking-wide text-center">What can The Foundry do?</p>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  <div className="p-3 bg-white border-4 border-black text-center">
                    <p className="text-xs font-black text-black">sharpen contrast saturation</p>
                  </div>
                  <div className="p-3 bg-white border-4 border-black text-center">
                    <p className="text-xs font-black text-black">vignette warm grain</p>
                  </div>
                  <div className="p-3 bg-white border-4 border-black text-center">
                    <p className="text-xs font-black text-black">blur soft dreamy</p>
                  </div>
                  <div className="p-3 bg-white border-4 border-black text-center">
                    <p className="text-xs font-black text-black">cool desaturate vintage</p>
                  </div>
                  <div className="p-3 bg-white border-4 border-black text-center">
                    <p className="text-xs font-black text-black">pixelate glitch noise</p>
                  </div>
                  <div className="p-3 bg-white border-4 border-black text-center">
                    <p className="text-xs font-black text-black">invert sepia grain</p>
                  </div>
                </div>
              </div>
            </>
            ...
            <>
              ...
              <div className="bg-white rounded-xl shadow-md p-4 mb-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="text-xs font-bold text-gray-600">
                    {stats.total} ready • {stats.completed} done
                  </div>
                </div>
                ...
                <div className="mb-3">
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="block text-xs font-bold text-gray-700 tracking-wide">
                      EFFECT COMBO {prompt && <span className="text-teal-600">({prompt.split(' ').length})</span>}
                    </label>
                    <div className="flex gap-1.5">
                      <button
                        onClick={() => setPrompt('')}
                        disabled={isProcessing || !prompt}
                        className="px-2.5 py-1 bg-gray-100 text-gray-600 text-xs font-bold rounded-lg hover:bg-gray-200 transition-all disabled:opacity-50 shadow-sm"
                      >
                        Clear
                      </button>
                      <button
                        ...
                        className="px-3 py-1 bg-purple-600 text-white text-xs font-bold rounded-lg hover:bg-purple-700 transition-all disabled:opacity-50 flex items-center gap-1.5 shadow-sm"
                      >
                  ...
                  <textarea
                    ...
                    placeholder="Type effects: sharpen vignette saturation, warm cool grain, etc."
                    className="w-full p-3 border-2 border-gray-200 rounded-lg focus:border-purple-500 focus:outline-none resize-none h-20 font-mono text-sm shadow-sm"
                    disabled={isProcessing}
                  />

                  {/* Example Effects */}
                  <div className="mt-3">
                    <p className="text-xs font-bold text-black mb-2 uppercase tracking-wide">Try these effects:</p>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      <button
                        onClick={() => setPrompt('sharpen contrast saturation')}
                        className="text-left p-2.5 bg-white hover:bg-brutal-yellow border-4 border-black transition-all text-xs font-bold"
                      >
                        sharpen contrast saturation
                      </button>
                      <button
                        onClick={() => setPrompt('vignette warm grain')}
                        className="text-left p-2.5 bg-white hover:bg-brutal-pink hover:text-white border-4 border-black transition-all text-xs font-bold"
                      >
                        vignette warm grain
                      </button>
                      <button
                        onClick={() => setPrompt('blur soft dreamy')}
                        className="text-left p-2.5 bg-white hover:bg-brutal-cyan border-4 border-black transition-all text-xs font-bold"
                      >
                        blur soft dreamy
                      </button>
                      <button
                        onClick={() => setPrompt('cool desaturate vintage')}
                        className="text-left p-2.5 bg-white hover:bg-brutal-yellow border-4 border-black transition-all text-xs font-bold"
                      >
                        cool desaturate vintage
                      </button>
                      <button
                        onClick={() => setPrompt('pixelate glitch noise')}
                        className="text-left p-2.5 bg-white hover:bg-brutal-pink hover:text-white border-4 border-black transition-all text-xs font-bold"
                      >
                        pixelate glitch noise
                      </button>
                      <button
                        onClick={() => setPrompt('invert sepia grain')}
                        className="text-left p-2.5 bg-white hover:bg-brutal-cyan border-4 border-black transition-all text-xs font-bold"
                      >
                        invert sepia grain
                      </button>
                  ...
                  {!isProcessing ? (
                    <>
                      <button
                        ...
                        className="flex-1 px-8 py-4 bg-brutal-yellow text-black text-base font-black uppercase border-4 border-black hover:bg-brutal-pink hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-brutal hover:shadow-brutal-hover hover:translate-x-1 hover:translate-y-1 tracking-tight"
                      >
                        <Play className="w-5 h-5" />
                        Process {stats.total} Images
                      </button>
                      <button
                        onClick={() => setShowSettings(!showSettings)}
                        className="px-4 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-lg transition-all flex items-center gap-1.5 text-sm shadow-sm"
                      >
                    ...
                    <button
                      ...
                      className="flex-1 px-6 py-3 bg-teal-600 hover:bg-teal-700 text-white text-base font-black rounded-lg transition-all flex items-center justify-center gap-2 shadow-md tracking-tight"
                    >
                ...
                {showSettings && (
                  <div className="border-t pt-3 mt-3">
                    <h3 className="font-black text-gray-900 mb-2 text-sm tracking-tight">EXPORT PRESETS</h3>
              ...
              <details className="mb-4">
                <summary className="bg-purple-50 rounded-lg p-3 cursor-pointer hover:bg-purple-100 transition-all shadow-sm">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Sparkles className="w-4 h-4 text-purple-600" />
                      <span className="font-black text-gray-900 text-sm tracking-tight">EFFECT LIBRARY</span>
                      <span className="text-xs text-gray-600 font-bold">(21 effects)</span>
                    </div>
                    <span className="text-gray-500 text-xs font-semibold">Expand</span>
                  </div>
                </summary>
                <div className="mt-3">
          ...
          {images.length > 0 && (
            <div className="space-y-4">
              ...
              {stats.completed > 0 && (
                <div className="bg-teal-50 border-2 border-teal-200 rounded-xl p-4 shadow-md">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="font-black text-gray-900 text-base mb-0.5 tracking-tight leading-tight">
                        {stats.completed} Images Ready
                      </h3>
                      <p className="text-xs text-gray-600 font-semibold">
                        Download all or export for Cricut/Etsy
                      </p>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={downloadAll}
                        className="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg font-bold transition-all flex items-center gap-1.5 text-sm shadow-sm"
                      >
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg font-bold transition-all flex items-center gap-1.5 text-sm shadow-sm"
                      >
              ...
              <div className="bg-white rounded-xl shadow-md p-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-black text-gray-900 text-base tracking-tight leading-none">YOUR IMAGES</h3>
                  <div {...getRootProps()} className="inline-block">
                    <input {...getInputProps()} />
                    <button className="px-3 py-1.5 bg-purple-100 hover:bg-purple-200 text-purple-700 rounded-lg font-bold transition-all flex items-center gap-1.5 text-xs shadow-sm">
                ...
                <div className="space-y-2 max-h-[500px] overflow-y-auto">
                  {images.map((image) => (
                    <div key={image.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg shadow-sm">
                      {/* Thumbnail */}
                        ...
                        {image.status === 'processing' && (
                          <div className="absolute inset-0 bg-black/50 rounded-md flex items-center justify-center">
                            <div className="relative">
                              ...
                              <div className="absolute inset-0 w-5 h-5 bg-white/20 rounded-full animate-ping" />
                            </div>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-gray-900 truncate text-sm leading-tight">{image.file.name}</p>
                        ...
                        {image.error && (
                          <p className="text-xs text-purple-600 mt-1 font-semibold">{image.error}</p>
                        )}
                      </div>
                      ...
                      <button
                        ...
                        className={`px-2 py-1 rounded-md font-bold text-xs transition-all shadow-sm ${
                        ...
                        title={!userHasPriorityQueue() ? 'Priority queue available on Creator+ tier' : 'Toggle priority'}
                      >
                        <Zap className="w-3 h-3 inline mr-0.5" />
                        {!userHasPriorityQueue() ? 'Pro' : image.priority === 'high' ? 'High' : 'Norm'}
                      </button>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {image.status === 'completed' && (
                          <>
                            ...
                            <button
                              ...
                              className="p-1.5 hover:bg-teal-100 rounded-md transition-all"
                        ...
                        <button
                          ...
                          className="p-1.5 hover:bg-red-100 rounded-md transition-all"
                        >
                          <X className="w-3.5 h-3.5 text-gray-600 hover:text-purple-600" />
                        </button>
                      </div>
                    </div>
                  ))}
