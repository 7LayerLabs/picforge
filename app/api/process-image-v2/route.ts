...
export async function POST(request: NextRequest) {
  logger.log('=== Process Image V2 API Called ===')
  ...
  try {
    ...
    logger.log('Content-Type received:', contentType)
    ...
    if (contentType.startsWith('application/json')) {
      ...
      logger.log('Parsing as FormData...')
      try {
        ...
        logger.log('FormData parsed successfully')
      } catch (formError) {
    ...
    logger.log('Processing with prompt:', prompt)
    logger.log('Image size (base64):', imageBase64.length)
