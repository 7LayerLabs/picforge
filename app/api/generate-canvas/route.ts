...
// Increase timeout for DALL-E API (can take 10-20 seconds)
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.info('Generating canvas with:', { prompt, size, quality })

    if (!prompt) {
      throw Errors.missingParameter('prompt')
    }

    logger.debug('Using OpenAI API key:', apiKey.substring(0, 10) + '...')
    ...
    logger.info('OpenAI response received:', response.data?.[0]?.url ? 'Image URL present' : 'No image URL')
